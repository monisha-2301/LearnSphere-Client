...
import { fetchCourseDetails, enrollCourse } from "../services/operations/courseDetailsAPI"
...
function CourseDetails() {
  ...
  const handleEnrollCourse = () => {
    ...
    if (token) {
      dispatch(enrollCourse(response?.data.courseDetails))
      return
    }
    setConfirmationModal({
      text1: "You are not logged in!",
        ...
        <div className="mx-auto box-content px-4 lg:w-[1260px] 2xl:relative ">
          <div className="mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-cente py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]">
            ...
            <div className="flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden">
              <button className="yellowButton" onClick={() => navigate(`/view-course/${courseId}/section/${response?.data?.courseDetails?.courseContent?.[0]?._id}/sub-section/${response?.data?.courseDetails?.courseContent?.[0]?.subSection?.[0]?._id}`)}>Start Learning</button>
              <button className="yellowButton" onClick={handleEnrollCourse}>Enroll Now</button>
            </div>
          </div>
